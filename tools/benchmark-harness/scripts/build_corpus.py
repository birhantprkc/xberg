...
def _language_sidecar() -> dict[str, str]:
    """`ground_truth/languages.json`, generated by `build_language_sidecar.py` (issue #1696's
    corpus work) -- absent from most checkouts, since it is an opt-in artifact the language
    calibration flow produces on demand, not part of the base acquire/normalize/gate pipeline.
    Cached: `_fixture` is called once per accepted doc, and re-reading + re-parsing the same
    JSON file per doc would be pure overhead. ~keep
    """
