...
- **(ocr): a binarization method too large to run on a small image now logs why it fell back to Otsu.** `binarization_method: "adaptive"` or `"sauvola"` silently fell back to Otsu when the image was smaller than the method's minimum tile/window size, with no way for the caller to tell their request was downgraded. Both fallback paths now emit a `WARN` naming the requested method, the image dimensions, and the fallback. (GH#1785)
